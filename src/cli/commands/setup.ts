...
const BETA_DIR = '.beta';
const CONFIG_PATH = path.join(BETA_DIR, 'config.toon');
const STATE_PATH = path.join(BETA_DIR, '.setup-state.json');
...
function writeConfig(state: SetupState): void {
  fs.mkdirSync(BETA_DIR, { recursive: true });
  fs.mkdirSync(path.join(BETA_DIR, 'logs'), { recursive: true });
  fs.mkdirSync(path.join(BETA_DIR, 'snapshots'), { recursive: true });
  ...
  const toonContent = `# betaclaw Configuration \u2014 generated by setup wizard\n# ${new Date().toISOString()}\n${encode('config', config as Record<string, string>)}`;
...
async function runSetupWizard(options: { reset?: boolean; mode?: string }): Promise<void> {
  ...
  if (options.reset) {
    ...
    console.log('\n  Configuration reset. Run "betaclaw setup" to reconfigure.\n');
    return;
  }
...
async function pairWhatsAppNow(): Promise<void> {
  ...
  try {
    ...
    console.log('  You can pair later by running "betaclaw start" — the listener stays open and will show a QR code.');
  }
}
...
async function runWizardSteps(rl: readline.Interface, state: SetupState): Promise<void> {
  ...
  if (state.step <= 1) {
    ...
    printDivider();
    console.log();
    console.log(`  ${clr.bold}Welcome!${R} This wizard will configure betaclaw in ~3 minutes.`);
  ...
  if (state.step <= 2) {
    ...
    printDivider();
    console.log();
    console.log('  How should betaclaw run actions on your system?');
    console.log();
    ...
    while (!valid) {
      ...
      console.log();
      console.log(`  ${clr.yellow}\u26A0  Full Control gives betaclaw host-level access.${R}`);
  ...
  if (state.step <= 3) {
    ...
    if (state.providers.length === 0) {
      console.log(`\n  ${clr.yellow}No providers configured.${R} You can add them later with ${cyan('betaclaw provider add')}.`);
    } else {
        ...
        if (ch) {
          if (ch.envVar) {
            ...
            if (pairNow.toLowerCase() !== 'n' && pairNow.toLowerCase() !== 'no') {
              ...
              console.log(`  WhatsApp will show QR on first ${cyan('betaclaw start')}.`);
            }
          }
        }
      }
  ...
  if (state.step <= 7) {
    ...
    const globalMemoryPath = 'betaclaw.md';
    try {
    ...
    printDivider();
    console.log();
    console.log(`  ${ok('\u2713')} Configuration saved to ${dim('.beta/config.toon')}`);
    ...
    console.log();
    console.log(`    ${cyan('betaclaw chat')}           Start chatting`);
    console.log(`    ${cyan('betaclaw start')}          Start daemon with all channels`);
    console.log(`    ${cyan('betaclaw doctor')}         Run health check`);
    console.log(`    ${cyan('betaclaw provider add')}   Add more AI providers`);
    console.log(`    ${cyan('betaclaw skills list')}    View available skills`);
    console.log();

    if (state.providers.length === 0) {
      console.log(`  ${clr.yellow}Note:${R} You still need to configure a provider before chatting.`);
      console.log(`  Run: ${cyan('betaclaw provider add')}`);
      console.log();
    }

    if (state.channels.length > 0) {
      console.log(`  Tip: Run ${cyan('betaclaw start')} to connect your messaging channels.`);
      console.log();
    }
